...
try:  # OpenCV may be unavailable in test environments
    import cv2  # type: ignore
except Exception:  # pragma: no cover - treat as optional dependency
...
def search_faces(
    ...
    for score, idx in zip(D[0], I[0]):
        ...
        if redis_client is not None:
            try:
                path = redis_client.hget(f"face:known:{fid}", "image_path")
                if isinstance(path, bytes):
                    path = path.decode()
                if path:
                    thumb = f"/faces/{Path(path).name}"
            except Exception:
                thumb = ""
        results.append({"id": fid, "score": float(score), "thumbnail_url": thumb})
    return results
...
def upsert_temp_face(temp_id: str, data: dict) -> None:
    """Upsert temporary unlabeled face data in Redis."""
    if redis_client is None:
        raise RuntimeError("redis not initialized")
    key = f"face:temp:{temp_id}"
    mapping = {
    ...
    if "last_seen" in data:
        redis_client.zadd("faces:unlabeled", {temp_id: float(data["last_seen"])})
