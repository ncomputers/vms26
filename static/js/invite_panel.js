...
if (form) {
  ...
  const startField = document.getElementById("visit_time");
  ...
  if (manualForm) {
    ...
    if (location.protocol !== "https:" && location.hostname !== "localhost") {
      if (cameraMsg)
        cameraMsg.textContent =
          "Camera access requires HTTPS. Please use https://";
    }
    ...
    manualForm.addEventListener("submit", async (e) => {
      ...
      if (photoInput && !noPhotoCb?.checked && !photoInput.value?.trim()) {
      ...
      const endVal = expiryField?.value || "";
      const startDt = startVal ? new Date(startVal) : null;
      let endDt = endVal ? new Date(endVal) : null;
      if (!startDt || isNaN(startDt.getTime())) {
        showFieldError(startField, "Start time is required");
        valid = false;
      }
      if (!endDt || isNaN(endDt.getTime())) {
        if (startDt) {
          ...
          showFieldError(expiryField, "End time is required");
          valid = false;
        }
      }
      if (startDt && endDt && startDt >= endDt) {
        showFieldError(expiryField, "End must be after start");
        valid = false;
      }
      if (!valid) return;
      const payload = {
        ...
        host_id: hostSel.getValue(true),
        company: form.querySelector('input[name="company"]')?.value || "",
        type: form.querySelector('select[name="visitor_type"]')?.value || "",
        purpose: document.getElementById("purpose")?.value || "",
        start_dt: startDt.toISOString(),
        end_dt: endDt.toISOString(),
        send_email: document.getElementById("sendMail")?.checked || false,
        no_photo: noPhotoCb?.checked || false,
      ...
      if (photoInput && photoInput.value && !noPhotoCb?.checked) {
        const bstr = atob(photoInput.value.split(",")[1] || photoInput.value);
        const u8 = Uint8Array.from(bstr, (c) => c.charCodeAt(0));
        data.append("photo", new Blob([u8], { type: "image/jpeg" }), "photo.jpg");
      }
      isSubmitting = true;
      updateCreateBtn();
      ...
      try {
        ...
        if (r.ok) {
          ...
          if (photoInput) photoInput.value = "";
          photoUploader?.reset();
          loadTable(true, { id: d.invite_id });
        } else {
          let msg = "Failed to create invite";
          try {
            const err = await r.json();
            if (err.detail) msg = Object.values(err.detail).join(", ");
          } catch {
            msg = await r.text();
          }
          showToast(msg || "Failed to create invite", "danger");
        }
      } catch (err) {
    ...
    document.getElementById("genLink").addEventListener("click", async () => {
      ...
      let valid = true;
      if (!host) {
        showFieldError(hostEl, "Host is required");
        valid = false;
      }
      if (!vtype) {
        showFieldError(typeEl, "Visitor type is required");
        valid = false;
      }
      if (!valid) return;
      const data = new FormData();
      ...
      btn.disabled = true;
      try {
        const r = await fetch("/invite/create?link=1", {
          method: "POST",
          body: data,
        });
        ...
        if (r.ok) {
          const d = await r.json();
          const fullLink = d.link.startsWith("http")
            ? d.link
            : `${location.origin}${d.link}`;
          box.innerHTML = `
        ...
        document.getElementById("linkBox").innerHTML =
          `<div class="alert alert-danger">${err.message || "Failed to generate link"}</div>`;
      } finally {
        updateLinkBtn();
      }
    });
    ...
    async function loadTable(reset = false, newInvite = null) {
      ...
      if (daysFilter?.value && daysFilter.value !== "all")
        params.append("days", daysFilter.value);
    ...
    async function hold(id) {
      await fetch("/invite/hold/" + id, { method: "PUT" });
      loadTable(true);
    }
    async function rejectInvite(id) {
      await fetch("/invite/reject/" + id, { method: "PUT" });
      loadTable(true);
    }
    ...
    form.addEventListener("submit", async (e) => {
      ...
      const r = await fetch("/invite/form/submit", {
        method: "POST",
        body: data,
      });
