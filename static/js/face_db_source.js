...
function updateStartState() {
  ...
  if (startBtn) startBtn.disabled = needPerm && permState === "denied";
  [scaleInput, neighborsInput, thresholdInput, sizeInput].forEach((i) => {
    if (i) i.disabled = needPerm && permState === "denied";
  });
...
export function initSource() {
  ...
  params = {
    scaleFactor: () => parseFloat(scaleInput?.value ?? defaults.scaleFactor),
    minNeighbors: () =>
      parseInt(neighborsInput?.value ?? defaults.minNeighbors, 10),
    threshold: () => parseFloat(thresholdInput?.value ?? defaults.threshold),
    minFaceSize: () =>
      parseInt(sizeInput?.value ?? defaults.minFaceSize, 10),
  };
