...
def face_db_page(request: Request, templates: Jinja2Templates = Depends(get_templates)):
...
def init_context(cfg: dict, redis_client, redis_facade=None) -> None:
...
def detect_faces(rgb_frame: np.ndarray) -> list[dict[str, Any]]:
    ...
    for f in faces:
        ...
        results.append({"box": [x1, y1, x2 - x1, y2 - y1], "confidence": score})
    return results
...
async def process_frame(
    ...
    if arr is None:
        raise HTTPException(status_code=400, detail="Image decode failed")
    rgb = cv2.cvtColor(arr, cv2.COLOR_BGR2RGB)
    if config.get("enable_face_matching", True):
        results = match_faces(rgb, threshold)
    else:
        results = detect_faces(rgb)
