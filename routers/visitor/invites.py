...
async def invites_create(
    ...
    """Create an invite via JSON or multipart form data."""
    from . import Path
    ...
    if ctype.startswith("multipart/"):
        ...
        if json_part:
            try:
                ...
                return JSONResponse({"detail": {"json": "Invalid JSON"}}, status_code=400)
    ...
    if not purpose:
        errors["purpose"] = "Purpose is required"
    if not start_dt:
        errors["start_dt"] = "Start datetime is required"
    if not end_dt:
        if start_dt:
            try:
                s = datetime.fromisoformat(start_dt)
                end_dt = (s + timedelta(minutes=60)).isoformat()
                data["end_dt"] = end_dt
            except Exception:
                errors["end_dt"] = "End datetime is required"
        else:
            errors["end_dt"] = "End datetime is required"
    try:
        ...
        if s_dt >= e_dt:
            errors["date_range"] = "start_dt must be before end_dt"
    except Exception:
        if "start_dt" not in errors:
            errors["start_dt"] = "Invalid start_dt"
        if "end_dt" not in errors:
            errors["end_dt"] = "Invalid end_dt"

    if errors:
        return JSONResponse({"detail": errors}, status_code=400)

    if not photo_bytes and data.get("photo"):
        try:
            photo_bytes = base64.b64decode(data.get("photo"))
        except Exception:
            return JSONResponse({"detail": {"photo": "Invalid photo"}}, status_code=400)

    today = datetime.now().strftime("%Y%m%d")
    invite_id = f"VST-{today}-{str(uuid.uuid4().int)[:4]}"
    ...
    rec = {
        ...
        "visitor_type": data.get("type", ""),
        "company": data.get("company", ""),
        "host": data.get("host_id", ""),
        "visit_time": start_dt,
        "expiry": end_dt,
        "purpose": purpose,
        "photo_url": photo_url if not data.get("no_photo") else "",
        "status": "scheduled",
        "invite_source": "api",
        "ts": int(time.time()),
    }
...
async def invite_public_submit(
    ...
    photo_waived: str = Form("off", alias="no_photo"),
    photo_waiver_reason: str = Form("", alias="no_photo_reason"),
    ctx: SimpleNamespace = Depends(get_context),
):
    if not isinstance(ctx, SimpleNamespace):
        ctx = get_context()
    ...
    if photo_waived != "on" and not photo and not rec.get("photo_url", "").strip():
        errors["photo"] = "Photo is required"
    if errors:
        return JSONResponse({"errors": errors}, status_code=400)
    ...
    if photo and photo.filename:
        ...
        photo_path = out_dir / f"{id}.{ext}"
        with open(photo_path, "wb") as f:
            f.write(img_bytes)
        photo_url = f"/invite_photos/{id}.{ext}"
    ...
    return {"ok": True, "redirect": redirect, "visitor_id": visitor_id}
...
@router.get("/invites/{iid}")
async def invites_get(
    iid: str,
    user: dict = Depends(require_viewer),
    ctx: SimpleNamespace = Depends(require_visitor_mgmt),
):
    """Alias for ``/invite/{iid}`` returning invite details."""
    return await invite_get(iid, user, ctx)
