...
try:  # OpenCV may be absent in tests
...
try:  # visitor module may have heavy deps; load lazily
...
def _validate_active_pass(redis, phone: str) -> None:
    """Reserve phone for gatepass creation or raise if one is active."""
    ts_int = int(time.time())
    key = f"gatepass:reserve:{phone}"
    try:
        ok = redis.set(key, "1", nx=True, ex=120)
...
async def gatepass_create(
    ...
    try:
        ...
        try:
            redis.delete(f"gatepass:reserve:{phone}")
        except Exception:
            pass
        return JSONResponse({"error": "redis_unavailable"}, status_code=503)
    ...
    try:
        ...
        redis.hset("gatepass:active_phone", phone, gate_id)
    except Exception:
        try:
            redis.delete(f"gatepass:reserve:{phone}")
        except Exception:
    ...
    try:
        ...
        return JSONResponse({"error": "redis_unavailable"}, status_code=503)
    try:
        save_master = getattr(visitor, "_save_visitor_master", None)
        if save_master:
            vid = save_master(
                name,
                email,
            ...
            vid = visitor_db.get_or_create_visitor(
                name, phone, email, company_name, image_url or ""
            )
    except Exception:
    ...
    if host:
        ...
        try:
            ...
            invalidate_cache = getattr(visitor, "invalidate_host_cache", None)
            if invalidate_cache:
                invalidate_cache()
        except Exception:
    ...
    if needs_approval == "on" and approver_email:
        ...
        qr_img = _qr_data_uri(qr_content)
        card_html = gatepass_service.render_gatepass_card(entry, qr_img)
        msg = (
            f"{card_html}"
