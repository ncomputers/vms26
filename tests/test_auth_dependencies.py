...
def test_invite_create_requires_viewer(tmp_path):
    app = _create_visitor_app(tmp_path)
    client = TestClient(app)
    resp = client.post("/invite/create", follow_redirects=False)
    assert resp.status_code == 302
    assert resp.headers["location"] == "/login"


def test_invite_list_requires_viewer(tmp_path):
    app = _create_visitor_app(tmp_path)
    client = TestClient(app)
    resp = client.get("/invite/list", follow_redirects=False)
    assert resp.status_code == 302
    assert resp.headers["location"] == "/login"


def test_invite_cctv_requires_viewer(tmp_path):
    app = _create_visitor_app(tmp_path)
    client = TestClient(app)
    resp = client.get("/invite/cctv", follow_redirects=False)
    assert resp.status_code == 302
    assert resp.headers["location"] == "/login"


def test_invite_lookup_requires_viewer(tmp_path):
    app = _create_visitor_app(tmp_path)
    client = TestClient(app)
    resp = client.get("/invite/lookup", params={"phone": "123"}, follow_redirects=False)
    assert resp.status_code == 302
    assert resp.headers["location"] == "/login"
