...
test('shows success toast, resets form and adds row on invite creation', async () => {
  document.body.innerHTML = `
    <form id="manualForm">
      ...
      <select id="purpose"><option value="Meeting" selected>Meeting</option></select>
      <select id="linkHost"></select>
      <select id="linkType"></select>
      <div class="photo-controls" data-prefix="p"><input type="hidden" value="img" /></div>
  ...
  global.intlTelInput = jest.fn(() => ({ isValidNumber: () => true, getNumber: () => '9876543210' }));
  ...
  global.fetch = jest.fn((url) => {
    if (url === '/invites') {
      return Promise.resolve({ ok: true, json: () => Promise.resolve({ invite_id: 'XYZ' }) });
    }
  ...
  expect(global.fetch).toHaveBeenCalledWith('/invites', expect.any(Object));
});
