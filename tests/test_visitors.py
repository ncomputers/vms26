...
def test_api_visitor_report_pagination(monkeypatch):
    ...
    data1 = asyncio.run(
        registration.api_visitor_report(
            page=1, page_size=2, include_pending=False, ctx=visitor.get_context()
        )
    )
    ...
    data2 = asyncio.run(
        registration.api_visitor_report(
            page=2, page_size=2, include_pending=False, ctx=visitor.get_context()
        )
    )
...
def test_api_visitor_report_view_grouping(monkeypatch):
    ...
    visitor_view = asyncio.run(
        registration.api_visitor_report(
            view="visitor", include_pending=False, ctx=visitor.get_context()
        )
    )
    ...
    host_view = asyncio.run(
        registration.api_visitor_report(
            view="host", include_pending=False, ctx=visitor.get_context()
        )
    )
