...
def test_yuvj_source_adds_full_range_filter(monkeypatch):
    captured = {}
    ...
    stream = FFmpegCameraStream("rtsp://demo", width=2, height=2, start_thread=False)
    cmd = captured["cmd"]
    assert "scale=in_range=pc:out_range=pc" in ",".join(cmd)
    assert cmd.index("-vf") < cmd.index("-pix_fmt")
    assert stream.full_range
    stream.release()
...
def test_raw_buffer_and_lazy_decode(monkeypatch):
    frames = b"\x00\x00\x00\x01aaaa\x00\x00\x00\x01bbbb"
    ...
    def fake_run(*a, **k):
        called["run"] = True
        return (b"\x00\x00\x00", b"")

    monkeypatch.setattr(
        ffmpeg,
        "input",
        lambda *a, **k: types.SimpleNamespace(
            output=lambda *a, **k: types.SimpleNamespace(run=fake_run)
        ),
    )

    assert not called["run"]
    ok, frame = stream.decode_latest()
    assert ok and isinstance(frame, np.ndarray)
    assert called["run"]
    assert isinstance(stream.queue[-1], bytes)
    stream.release()
