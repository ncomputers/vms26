...
def admin_client(monkeypatch, tmp_path):
    config = {"users": []}
    admin_users.init_context(config, None, str(tmp_path), str(tmp_path / "cfg.json"))
    app = FastAPI()
    app.include_router(admin_users.router)
    app.dependency_overrides[module_utils.require_admin] = lambda: None
...
def test_mfa_toggle(admin_client):
    ...
    assert admin_users.cfg["users"][0]["mfa_enabled"] is False
    r = admin_client.put("/admin/users/bob", json={"mfa_enabled": True})
    assert r.status_code == 200
    assert admin_users.cfg["users"][0]["mfa_enabled"] is True
