...
from modules.tracker.face_tracker import FaceTracker
...
def test_face_tracker_flow(monkeypatch):
    ...
    monkeypatch.setattr(
        "modules.tracker.face_tracker._frontalness", lambda face: 0.5
    )
    det = _make_detector(
        [
            (20, 10, 50, 40, 0.8),
...
def test_camera_manager_starts_face_tracker():
    ...
    async def run():
        ...
        mgr = CameraManager(
            ...
            r,
            lambda: cams,
            lambda cam, cfg, tr, r: tr.setdefault(cam["id"], object()),
